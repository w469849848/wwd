...
<meta name="keywords" content="选择业务员" />
<meta name="description" content="选择业务员" />
			...
			<div class="filter-wrap clearfix">
				<form id="limitPageForm" method="post">
					<input type="hidden" name="page.index" value="${page.index}" /> <input
						...
						class="filter border-radius bg-color" placeholder="中文名"
						name="sSalChineseName" value="${sSalChineseName }" id="filter" type="text" />
					</label>
				</form>
				...
				<tbody>
					<c:forEach items="${datas.DataList}" var="d">
						<tr saleId="${d.sSaleId }" salChinName="${d.sSalChineseName }">
							...
							<td>${d.sSalOrgName}</td>
							<td><span class="state"> <c:if test="${d.nTag ==0}">
										<img
											src="${pageContext.request.contextPath}/resources/egolm/assets/images/close.png" /></span>未审核
		...
		function clicked(id) {
			...
			for (i = 0; i < o.length; i++) {
				o[i].ondblclick = function() { //设置事件
					...
					var salChinName = $(this).attr("salChinName");
					parent.getSaleId(saleId, salChinName,"${saleNO}");
