	...
	<table class="Select" id="SelectList" style="width:100%;">
		<thead>
			<tr style='height:28px;'>
				<c:if test="${promo.sSettingModeID eq 'G' || isGift}">
					<th>商品编号</th>
					<th>商品名称</th>
					<th>商品规格</th>
					<th>商品单位</th>
				...
				<c:if test="${promo.sSettingModeID eq 'B' && !isGift}">
					<th>品牌编号</th>
					<th>品牌名称</th>
				</c:if>
				<c:if test="${promo.sSettingModeID eq 'C' && !isGift}">
					<th>分类编号</th>
					<th>分类名称</th>
				</c:if>
				<th>操作</th>
			...
			success : function(json) {
				if(json.IsValid) {
					...
					$(json.datas).each(function() {
						var html = "";
						if(promo.sSettingModeID == 'G' || ${isGift == true}) {
							...
							html += "</tr>";
						} else if(promo.sSettingModeID == 'B') {
							html += "<tr class='rows' id='ID" + this.sBrandID + "'>";
							html += "<td>" + this.sBrandID + "</td>";
							html += "<td>" + this.sBrandName + "</td>";
							html += "<td><a href=\"javascript:select('" + this.sBrandID + "');\">选择</a></td>";
							html += "</tr>";
						} else if(promo.sSettingModeID == 'C') {
							html += "<tr class='rows' id='ID" + this.sCategoryNO + "'>";
							html += "<td>" + this.sCategoryNO + "</td>";
							html += "<td>" + this.sCategoryDesc + "</td>";
							html += "<td><a href=\"javascript:select('" + this.sCategoryNO + "');\">选择</a></td>";
							html += "</tr>";
						}
