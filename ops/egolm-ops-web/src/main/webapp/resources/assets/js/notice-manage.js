jQuery(function($) {
	...
	function changeNum(){
		//汉字的个数 
		var str = ($tex.val().replace(/\w/g,"")).length; 
		//非汉字的个数 
		var abcnum = $tex.val().length-str; 
		var total = str*2+abcnum; 
		$(".txt-limt").find("span").html(total);
	}
	//汉字的个数 
	var str2 = ($tex.val().replace(/\w/g,"")).length; 
	//非汉字的个数 
	var abcnum2 = $tex.val().length-str2; 
	var total2 = str2*2+abcnum2; 
	$(".txt-limt").find("span").html(total2);
});
